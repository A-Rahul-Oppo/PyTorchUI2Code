...
##
epoch_list_acc_train = {i: [] for i in range(10)}
epoch_list_los_train = {i: [] for i in range(10)}
##
    ...
    for epoch in range(num_epochs):
        ...
        for batch_datas, batch_targets, batch_targets_eval, _ in train_loader:
            ...
            if optim.training_step % 50 == 0:
                ...
                ##
                epoch_list_acc_train[epoch].append(100 * (num_corrects / num_nonzeros))
                epoch_list_los_train[epoch].append(step_loss)
                print(epoch_list_acc_train[epoch][-1:])
                print(epoch_list_los_train[epoch][-1:])
                ##
...
def main():
    ...
    with open('convert_acc.txt', 'w') as convert_file:
        convert_file.write(json.dumps(epoch_train_acc))
    with open('convert_los.txt', 'w') as convert_file:
        convert_file.write(json.dumps(epoch_train_los))
